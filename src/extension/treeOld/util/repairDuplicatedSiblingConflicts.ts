...
export async function repairDuplicatedSiblingConflicts<TNode extends ITreeNode>(
    ...
    const candidateValues = siblings
        .filter((node) => candidateIdSet.has(node.id))
        .map((node) => getNodeValueByPath(node, uniqueBy))
        .filter((value) => value !== undefined)
    if (candidateValues.length === 0) return

    const groups: { value: any; nodes: TNode[] }[] = []
    for (const node of siblings) {
        const value = getNodeValueByPath(node, uniqueBy)
        if (value === undefined || !candidateValues.some((candidateValue) => isTreeUniqueValueEqual(candidateValue, value))) continue
        const group = groups.find((item) => isTreeUniqueValueEqual(item.value, value))
        if (group) {
            group.nodes.push(node)
        } else {
            groups.push({ value, nodes: [node] })
        }
    }
