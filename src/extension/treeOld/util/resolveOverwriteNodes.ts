...
/** 单次 `$in` 查询最多携带的值数量，避免大批量同步超过数据库参数上限。 */
const OVERWRITE_VALUE_BATCH_SIZE = 500
...
export async function resolveOverwriteNodes<TNode extends ITreeNode>(
    ...
    const hasComplexConflictValue = conflictValues.some((value) => value !== null && typeof value === "object")
    // 适配器对对象、日期等复杂值的 `$in` 支持并不统一；复杂唯一键改为读取同级后用深比较，保证语义一致。
    let conflictNodes: TNode[] = []
    if (conflictValues.length > 0) {
        if (hasComplexConflictValue) {
            conflictNodes = await table.findMany({ parentId }) as TNode[]
        } else {
            const nodesById = new Map<string, TNode>()
            for (let index = 0; index < conflictValues.length; index += OVERWRITE_VALUE_BATCH_SIZE) {
                const valueBatch = conflictValues.slice(index, index + OVERWRITE_VALUE_BATCH_SIZE)
                const batchNodes = await table.findMany({ parentId, [uniqueBy]: { $in: valueBatch } }) as TNode[]
                for (const node of batchNodes) nodesById.set(node.id, node)
            }
            conflictNodes = Array.from(nodesById.values())
        }
    }
    ...
    for (const node of resolvedNodes) {
        ...
        const conflicts = conflictNodes.filter((item) => {
            return !ignoreNodeIds.has(item.id) && item.id !== node.id && isTreeUniqueValueEqual(getNodeValueByPath(item, uniqueBy), value)
        })
...
function resolveReplaceBatchConflicts<TNode extends ITreeNode>(nodes: TNode[], uniqueBy: string): TNode[] {
    ...
    for (const node of nodes) {
        ...
        const duplicatedIndex = resolvedNodes.findIndex((resolvedNode) => {
            const resolvedValue = getNodeValueByPath(resolvedNode, uniqueBy)
            return resolvedValue !== undefined && isTreeUniqueValueEqual(resolvedValue, value)
        })
        if (duplicatedIndex >= 0) {
            // 保留最后一个写入意图，同时维持第一个冲突键在批次中的相对位置。
            resolvedNodes[duplicatedIndex] = node
        } else {
            resolvedNodes.push(node)
        }
    }
