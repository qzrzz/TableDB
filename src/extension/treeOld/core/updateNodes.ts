...
import { assertNotMoveIntoSelfOrDescendant, assertTreeNodeName, assertTreeParentExists, assertTreeParentId } from "../util/assertTreeParent"
...
async function assertUpdateOpSafe(
    ...
    if (setOp.parentId !== undefined) {
        assertTreeParentId(setOp.parentId)
        if (options?.deep) {
...
function assertRequiredSetFieldValues(setOp?: Record<string, any>): void {
    ...
    if (Object.prototype.hasOwnProperty.call(setOp, "size") && (typeof setOp.size !== "number" || !Number.isFinite(setOp.size) || setOp.size < 0)) {
        throw new Error("[TableTree] updateNodes 不能把树结构字段 size 更新为空值")
    }
    if (Object.prototype.hasOwnProperty.call(setOp, "index") && typeof setOp.index !== "string") {
        throw new Error("[TableTree] updateNodes 不能把树结构字段 index 更新为非字符串")
    }
    if (Object.prototype.hasOwnProperty.call(setOp, "modif") && (typeof setOp.modif !== "number" || !Number.isFinite(setOp.modif))) {
        throw new Error("[TableTree] updateNodes 的 modif 必须是有限数字")
    }
}
...
function assertCompareStructureFieldsSafe(compareOp: Record<string, any> | undefined, operator: "$min" | "$max"): void {
    ...
    for (const [key, value] of Object.entries(compareOp)) {
        ...
        if (key === "size" && (typeof value !== "number" || !Number.isFinite(value) || value < 0)) {
