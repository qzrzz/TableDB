...
export class TableTree<TNode extends ITreeNode = ITreeNode> extends Table<TNode> {
    /** 当前实例的树操作队列，避免同一个实例上的异步操作交叉执行。 */
    private treeOperationTail: Promise<void> = Promise.resolve()

    private async runTransaction<T>(fn: () => Promise<T>): Promise<T> {
        let release!: () => void
        const previousOperation = this.treeOperationTail
        this.treeOperationTail = new Promise<void>((resolve) => {
            release = resolve
        })
    ...
    /** 让树工具复用同一实例的串行队列和适配器事务。 */
    async runTreeTransaction<T>(fn: () => Promise<T>): Promise<T> {
